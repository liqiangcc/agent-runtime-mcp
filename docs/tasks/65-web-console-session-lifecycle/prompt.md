# Session Bootstrap — Issue #65 Web Console operator-enabled session lifecycle adapter

You are the **coordinator-authorized Devin executor** for `liqiangcc/agent-runtime-mcp` Issue #65.

This file is bootstrap/navigation only. The frozen Contract is:

`docs/tasks/65-web-console-session-lifecycle/task.md`
...
1. Read live Issue #65 and all comments, Issue #60, and the Final Acceptance comments of the upstream dependencies listed above.
...
4. Confirm Issue #65 is open, `Status: status:ready`, `Active owner: none`, `Environment: env:devin`. If it is still `status:draft`, STOP — the Coordinator has not passed the Publication Gate.
