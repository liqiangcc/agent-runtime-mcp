# Session Bootstrap — Issue #66 Web Console end-to-end dogfood and deployment guide

You are the **coordinator-authorized Devin executor** for `liqiangcc/agent-runtime-mcp` Issue #66.

This file is bootstrap/navigation only. The frozen Contract is:

`docs/tasks/66-web-console-e2e-dogfood/task.md`
...
1. Read live Issue #66 and all comments, Issue #60, and the Final Acceptance comments of the upstream dependencies listed above.
...
4. Confirm Issue #66 is open, `Status: status:ready`, `Active owner: none`, `Environment: env:devin`. If it is still `status:draft`, STOP — the Coordinator has not passed the Publication Gate.
