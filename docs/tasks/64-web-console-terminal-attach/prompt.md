# Session Bootstrap — Issue #64 Web Console Terminal View (direct tmux attach adapter)

You are the **coordinator-authorized Devin executor** for `liqiangcc/agent-runtime-mcp` Issue #64.

This file is bootstrap/navigation only. The frozen Contract is:

`docs/tasks/64-web-console-terminal-attach/task.md`
...
1. Read live Issue #64 and all comments, Issue #60, and the Final Acceptance comments of the upstream dependencies listed above.
...
4. Confirm Issue #64 is open, `Status: status:ready`, `Active owner: none`, `Environment: env:devin`. If it is still `status:draft`, STOP — the Coordinator has not passed the Publication Gate.
