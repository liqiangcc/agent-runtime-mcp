# Task 61 — Web Console skeleton, Tailscale bind guard, MCP client adapter and session list
...
Create the `console/` upper-layer component with (1) an HTTP server that may only listen on loopback or a
local Tailscale address (no application-level authentication; tailnet = access boundary), (2) an MCP client adapter that talks to `agent-runtime-mcp` over stdio using
...
Preconditions:
  ...
  - Console started by the operator with CONSOLE_BIND = 127.0.0.1 (default) or the host's Tailscale address
Main flow:
  1. browser on the tailnet loads the Console at the Tailscale address
...
tailnet access control | Console      → Tailscale membership/ACL decides who; the Console authenticates nobody
...
```text
console/server            = HTTP/WS ingress, bind guard, Origin/Host check, static UI serving
...
Control (outside this Task / operator): when the MCP/tmux exist, restart policy, tailnet membership/ACLs, deployment topology.

## Success / Failure / Degradation

- Success proves: list/health are served through the MCP only; bind guard refuses non-loopback/non-Tailscale addresses; boundary guards pass.
- Hard failure: any `src/` change; any tmux invocation from `console/` in this Task; listening on 0.0.0.0/:: or any non-loopback, non-Tailscale address; secrets in logs.
...
access boundary              → Tailscale bind guard in console/server (no roles, no auth middleware)
...
- Bind guard: `CONSOLE_BIND` (default `127.0.0.1`) and `CONSOLE_PORT`. Startup succeeds only if the address is loopback, or is in `100.64.0.0/10` / `fd7a:115c:a1e0::/48` **and** is currently assigned to a local network interface (`os.networkInterfaces()`). `0.0.0.0`, `::`, and any other address make the server exit non-zero with a clear message. No override flag. `Origin`/`Host` of browser requests must match the bound address:port for WS/SSE and state-changing routes.
...
- Structured logging without terminal payloads; no request bodies logged.
- CI: new job(s) for `console/` typecheck + unit tests + one integration test that prepares a disposable tmux server (test harness only), builds the repo, starts the Console on loopback, and asserts: startup refused for `0.0.0.0` and for a non-Tailscale LAN address, list matches `tmux list-panes` count within the allowlist, health banner true, then after `tmux kill-server` health false and no session recreation.
...
- User authentication, roles, TLS, reverse proxy — by Coordinator decision the Tailscale tailnet is the access boundary; document only.
...
- Access control is the tailnet's (deployment layer, `docs/deployment.md §5`); the Console's only control is the bind guard, which must not be weakened by later Tasks.
...
3. Expose an internal HTTP JSON API (`/api/health`, `/api/channels`, `/api/channels/:id`); the UI consumes only this API.
4. Enforce the bind guard at startup; document every env var and the tailnet trust model in `console/README.md`.
...
```text
C1: startup is refused for `0.0.0.0`, `::` and a non-Tailscale non-loopback address; accepted for `127.0.0.1` and for a `100.64.0.0/10` address present on a local interface (unit with mocked interfaces + integration on loopback). Mismatched Origin/Host on a state-changing route or WS is rejected. (unit)
C2: list reflects MCP list_channels for a disposable tmux server with TMUX_ALLOWED_SESSIONS applied; disallowed session is absent. (integration, real tmux, Actions)
...
C7: logs contain no terminal payload. (unit test on logger + review)
```
...
Threats/controls: T3 (scope: rely on MCP scope, same TMUX_* env), T5 (no payload logging), T8 (no inference); access control delegated to the Tailscale tailnet per docs/web-console-requirements.md §8 — everyone on the tailnet has full Console capability
Remote ingress affected: the Console is itself a human ingress on the tailnet; it refuses to listen anywhere except loopback or a Tailscale address
```
...
4. SC4: `console/README.md` documents the bind guard, the tailnet trust model (no Console-side auth), env vars, and states the Console is not part of the MCP product.

## Failure / Blocked Rules

FAIL: any Success Criterion not met. BLOCKED: MCP client v2 cannot be used from `console/` without changing product code, or Actions cannot provide a real tmux runner. Resume when the blocker is resolved by the Coordinator. Never weaken the bind guard to pass.
