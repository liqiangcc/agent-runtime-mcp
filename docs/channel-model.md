...
The accepted v0.2.1 continuity amendment (Issue #38) sizes bounded history for the entire five-minute cursor lease at the nominal 250 ms cadence: 1,536 records and 256 KiB of logical event payload per observer. The count margin covers the 1,200 nominal changes plus baseline/lease-edge placement, in-flight completion at both edges and an explicitly measured scheduler-jitter envelope; tests model starts and completions rather than assert a universal inter-start minimum. Logical payload bytes are not JavaScript heap bytes: eight observers consume at most 2 MiB of ring budget, with supported-Node/V8 measurement (environment and method reported) for the separate 2 MiB allowance for metadata and runtime overhead under the existing 4 MiB retained-state ceiling. Field-width sizing includes the 15-minute observer sequence range, fractional monotonic values and full ISO wall-clock values. The cursor remains unrenewed; after TTL expiry or a true continuity gap, fresh observe starts a new baseline and cannot report past activity.
