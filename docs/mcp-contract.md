...
Issue #38 proposes a v0.2.1 bounded-continuity amendment, pending implementation and Coordinator review: retain 1,536 change records and 256 KiB of logical event payload per observer. Four nominal sample opportunities per second over the five-minute (300 s) cursor lease yield 1,200 changes; the additional 336-record margin covers baseline/lease-edge placement, in-flight completion at both edges and the explicitly measured scheduler-jitter envelope. Tests must instrument starts/completions and model that envelope; they must not assert an exact minimum between every timer callback. Completion overrun above the existing 1,000 ms gap bound remains an explicit continuity failure. The 256 KiB figure accounts for logical serialized event payload only, not JavaScript heap; field-width evidence must include the 15-minute observer sequence range, fractional monotonic values and full ISO wall-clock values. The proposed retained-state budget is 2 MiB for eight ring payloads plus 2 MiB for observer/token/waiter metadata and runtime/allocator overhead. A supported-Node/V8 forced-GC measurement may supply implementation evidence for that overhead, but is not a universal guarantee for all runtimes and must identify its environment and method. If that proof fails, this capacity plan cannot publish and a separately reviewed bounded aggregate is required. Cursor TTL is unchanged, waits do not renew it, timeout preserves the input cursor, and expiry or real continuity loss still produces explicit errors; fresh observe cannot acknowledge activity that occurred before it.
