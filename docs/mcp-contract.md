...
Issue #32's additive contract is accepted and integrated in the current main runtime; the verified public surface below exposes seven tools.
...
Issue #38's accepted v0.2.1 bounded-continuity contract retains 1,536 change records and 256 KiB of logical event payload per observer. Four nominal sample opportunities per second over the five-minute (300 s) cursor lease yield 1,200 changes; the additional 336-record margin covers baseline/lease-edge placement, in-flight completion at both edges and the explicitly measured scheduler-jitter envelope. Tests instrument starts/completions and model that envelope; they do not assert an exact minimum between every timer callback. Completion overrun above the existing 1,000 ms gap bound remains an explicit continuity failure. The 256 KiB figure accounts for logical serialized event payload only, not JavaScript heap; field-width evidence includes the 15-minute observer sequence range, fractional monotonic values and full ISO wall-clock values. The retained-state budget is 2 MiB for eight ring payloads plus 2 MiB for observer/token/waiter metadata and runtime/allocator overhead. Supported-Node/V8 forced-GC evidence supplies implementation measurement for that overhead, but is not a universal guarantee for all runtimes. If that proof fails, the capacity plan is blocked pending a separately reviewed bounded aggregate. Cursor TTL is unchanged, waits do not renew it, timeout preserves the input cursor, and expiry or real continuity loss still produces explicit errors; fresh observe cannot acknowledge activity that occurred before it.
