...
Issue #38 proposes a v0.2.1 bounded-continuity amendment, pending implementation and Coordinator review: retain 1,536 change records and 256 KiB of logical event payload per observer. Four sample starts per second over the five-minute (300 s) cursor lease yield at most 1,200 changes; the additional 336-record margin covers lease-edge/baseline boundaries and measured scheduler jitter. Tests must verify monotonic sample-start deltas are never below the fixed 250 ms cadence and must inject completion jitter; an overrun above the existing 1,000 ms gap bound remains an explicit continuity failure. The 256 KiB figure accounts for logical serialized event payload only, not JavaScript heap. The proposed retained-state budget is 2 MiB for eight ring payloads plus 2 MiB for observer/token/waiter metadata and runtime/allocator overhead; a forced-GC heap measurement is required to prove the 4 MiB total. If that proof fails, this capacity plan cannot publish and a separately reviewed bounded aggregate is required. Cursor TTL is unchanged, waits do not renew it, timeout preserves the input cursor, and expiry or real continuity loss still produces explicit errors; fresh observe cannot acknowledge activity that occurred before it.
