...
export interface ObservationSampler { sample(channelId: string): Promise<ObservationSample>; }
...
export class ObservationManager {
      ...
      return await new Promise<WaitChannelEventResult>((resolve, reject) => {
        ...
        const evaluate = async () => {
          ...
          try {
            ...
            if (last && now - last.at >= idle && now < deadline) return finish(() => resolve(this.result(observer, 'output_idle', input, idle, timeout, last.seq, true, events[0], last)));
        ...
        observer.waiters.add(wake); signal?.addEventListener('abort', abort, { once: true }); void evaluate();
      });
  ...
  private scheduleSample(observer: Observer): void {
    ...
    observer.sampling = true; this.activeSamples += 1;
    void this.sample(observer).finally(() => { observer.sampling = false; this.activeSamples -= 1; this.maybeDispose(observer); });
  }
  private async sample(observer: Observer): Promise<void> {
    ...
    try {
      const sample = await this.sampler.sample(observer.channelId); const finished = this.now();
  ...
  private issue(observer: Observer, seq: number): ObservationLease { const exp = Math.min(this.now() + LEASE_MS, observer.created + LIFETIME_MS); const token = randomBytes(24).toString('base64url'); this.tokens.set(token, { observer: observer.id, channelId: observer.channelId, instance: observer.instance, seq, exp }); return { cursor: token, channel_instance: observer.instance, model: 'snapshot_change', issued_at: new Date(this.wall()).toISOString(), valid_until: new Date(this.wall() + (exp - this.now())).toISOString(), continuity: 'complete' }; }
