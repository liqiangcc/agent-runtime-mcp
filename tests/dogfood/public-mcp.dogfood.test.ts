...
test('official MCP client dogfoods the complete public Channel surface', { timeout: 15000 }, async () => {
  ...
  // Endpoint lifecycle and shell setup remain external to Channel MCP. Use the
  // same reduced environment as StdioClientTransport, then disable terminal echo
  // so marker assertions prove observed command output rather than echoed input.
  ...
  try {
    ...
    // The upper-layer fixture destroys the endpoint. The still-connected MCP must
    // return a mechanical failure and must not recreate the missing session/pane.
